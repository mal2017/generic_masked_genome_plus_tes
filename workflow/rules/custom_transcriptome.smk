rule get_te_gtf:
    input:
        fasta = rules.get_custom_repeat_fasta.output.fasta,
    output:
        fai = rules.get_custom_repeat_fasta.output.fasta + '.fai',
        bed = 'results/masked_genome_plus_tes/{asm}/custom_transcriptome/REPEAT_FASTA.bed',
        gp = "results/masked_genome_plus_tes/{asm}/custom_transcriptome/REPEAT_FASTA.gp",
        gtf = 'results/masked_genome_plus_tes/{asm}/custom_transcriptome/REPEAT_FASTA.consensus.gtf',
    conda:
        "../envs/te_gtf.yaml"
    shell:
        """
...
rule make_transcripts_and_consensus_tes_gtf:
    ...
    output:
        gtf = "results/masked_genome_plus_tes/{asm}/transcriptome_plus_tes.gtf",
    conda:
...
rule make_transcripts_and_consensus_tes_tx2gene:
    ...
    output:
        tx2id = "results/masked_genome_plus_tes/{asm}/transcriptome_plus_tes.tx2id.tsv",
    conda:
        "../envs/rtracklayer.yaml"
    script:
...
rule make_transcripts_and_consensus_tes_fa:
    ...
    output:
        fasta = "results/masked_genome_plus_tes/{asm}/transcriptome_plus_tes.fasta",
    shell:
        """
